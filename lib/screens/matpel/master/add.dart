...
class AddMatpel extends StatefulWidget {
  
  @override
  _AddMatpelState createState() => _AddMatpelState();
}

class _AddMatpelState extends State<AddMatpel> {
  ...
  void storeUser() async {
    ...
    final response = await http.post(Uri.parse("${Env.URL_PREFIX}/matpel"), body: {
      "kelompok": tingkat,
    ...
    if (response.statusCode == 201) {
      final response2 = await http.post(Uri.parse("${Env.URL_PREFIX}/matpel/mapping"), body: {
        "id_matpel": json.decode(response.body)['data']['id'].toString(),
