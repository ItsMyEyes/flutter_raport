...
class GuruIndex extends StatelessWidget {
  String _filePath;
  ...
  void uploadFile() async {
    ...
    if(result != null) {
      ...
      var request = http.MultipartRequest("POST", Uri.parse("${Env.URL_PREFIX}/guru/upload"));
  ...
  Future<List<Guru>> getGuru() async {
    ...
    final response = await http.get(Uri.parse("${Env.URL_PREFIX}/guru?ta=${pref.getString('ta')}"), headers: requestHeaders);
    print("${Env.URL_PREFIX}/guru?ta=${pref.getString('ta')}");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: SpeedDial(
          ...
          children: [
            SpeedDialChild(
              ...
              onTap: () => {
                  Navigator.push(context,CupertinoPageRoute(builder: (context) => AddGuru()))    
              }
            ),
            SpeedDialChild(
      ...
      body: FutureBuilder(
            future: getGuru(),
            builder: (BuildContext context, AsyncSnapshot<List<Guru>> snapshot) {
              if (snapshot.hasError) {
                ...
                return _buildListView(context,gurus);
              } else {
                return Center(
...
Widget _buildListView(context,List<Guru> gurus) {
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(
                      ...
                      child: Text(
                        "Manage pengguna yang berole access guru, wali kelas, Admin, dan wakil kurikulum",
                      ),
                    ),
                    SizedBox(
                      width: size.width * .5, // it just take the 50% width
                      child: SearchBar(data: AddGuru(), text: 'Tambahkan guru'),
                    ),
                    SizedBox(height: 20),
                    Text(
                      "List Guru",
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        Guru guru = gurus[index];
                        return GestureDetector(
                          onTap: () {
                            Navigator.push(
                              context,
                              CupertinoPageRoute(
                                  builder: (context) => EditProfilePage(kode_login: guru.kode_login)));
                          },
                          onLongPress: () {
                            print("Long");
                          },
